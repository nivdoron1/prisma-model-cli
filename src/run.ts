...
function generateErrorDto(): void {
  const errorDtoPath = path.join('src', 'common', 'dto');
  const errorDtoFile = path.join(errorDtoPath, 'error-response.dto.ts');

  if (!fs.existsSync(errorDtoFile)) {
    fs.mkdirSync(errorDtoPath, { recursive: true });
...
`;
    fs.writeFileSync(errorDtoFile, content);
...
*/
function mapPrismaTypeToTs(fieldName: string, modelName: string, isRelation: boolean): string {
  if (isRelation) {
...
function generateDtos(model: string, ext: string) {
  ...
  for (const line of lines) {
    ...
    if (tsType.includes('Prisma.')) usesPrisma = true;
